...
//    Write;
    ...
    for(int i=0;i<n;i++)
    {
        sf("%d",&val[i]);
        nval[i]=val[i];
        if(val[i]<=m) cnt[val[i]]++,tot++;
    }
    int mn=n/m;
    int cn=n%m;
    int ex=n-tot;
    for(int i=1;i<=m && ex;i++)
    {
        if(cnt[i]<mn)
        {
            int p=0;
            while(ex>0&&cnt[i]<mn) ex--,cnt[i]++,p++;
            for(int j=0;j<n&&p>0;j++)
            {
                if(nval[j]>m) nval[j]=i,p--;
            }
        }
    }
    for(int i=0;i<MAX;i++)
    {
        if(cnt[i]>mn) ex+=cnt[i]-mn;
    }
    for(int i=1;i<=m && ex;i++)
    {
        if(cnt[i]<mn)
        {
            ...
            for(int j=0;j<n&&p>0;j++)
            {
                if(cnt[nval[j]]>mn) cnt[nval[j]]--,nval[j]=i,p--;
            }
        }
    }
    int chng=0;
