...
int bins(int lb,int ub,int item)
{
    int mid=(lb+ub)>>1;
    while(lb<=ub)
    {
        if(v[mid]>item) ub=mid-1;
        else lb=mid+1;
        mid=(lb+ub)>>1;
    }
    if(v[mid]<=item) return sum[mid]+data[v[mid]];
    else return -1;
}
...
int main()
{
    ...
    {
        cin>>num;
        if(data.find(num)==data.end()) data[num]=1,v.psb(num);
        else data[num]++;
    }
    sort(v.begin(),v.end());
    sum.psb(0);
    ...
    {
        sum.psb(sum[i-1]+data[v[i-1]]);
    }
    int mn=100000;
    for(int i=0;i<v.size();i++)
    {
        int ret=bins(0,v.size()-1,v[i]*2);
        if(ret!=-1)
        {
            ret=sum[i]+(sz-ret);
            mn=min(ret,mn);
