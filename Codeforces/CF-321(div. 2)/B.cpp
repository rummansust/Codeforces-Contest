...
//    Write;
    ...
    sort(all(val));
    for(int i=0;i<=n;i++) val1.psb(val[i].fs);
    for(int i=1;i<=n;i++)
    {
        csum[i]+=csum[i-1]+val[i].sc;
    }
    ...
    for(int i=1;i<=n;i++)
    {
        int l=upper_bound(all(val1),val1[i]+d-1)-val1.begin();
        if(l>=n) l=n-1;
        if(l<=0) continue;
        if(val1[l]>=val1[i]+d) l--;
        if(l<=0) continue;

        mx=max(mx,csum[l]-csum[i-1]);
