...
//    Write;
    ...
    for(int i=0;i<r;i++) sf("%s",&str[i]);
    for(int i=0;i<r;i++)
    {
        for(int j=0;j<c;j++)
        {
            if(str[i][j]=='#') continue;
            if(j+1<c && str[i][j+1]=='.') mat[i+1][j+1]++;
            if(i+1<r && str[i+1][j]=='.') mat[i+1][j+1]++;
        }
    }
    for(int i=0;i<r;i++)
    {
    ...
    while(q--)
    {
        ...
        int res=mat[x2][y2]-mat[x1-1][y2]-mat[x2][y1-1]+mat[x1-1][y1-1];
        ///border check
        for(int i=x2-1,j=y1-1;j<y2;j++)
        {
            if(i+1<r && str[i][j]=='.'&&str[i+1][j]=='.') res--;
        }
        for(int i=x1-1,j=y2-1;i<x2;i++)
        {
            if(j+1<c && str[i][j]=='.'&&str[i][j+1]=='.') res--;
        }
        pf("%d\n",res);
    }
