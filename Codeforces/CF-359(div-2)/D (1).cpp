...
int dfs(int u,int par)
{
    ...
    for(int i=0; i<SZ(adj[u]); i++)
    {
        ...
        if(!level[v])
        {
            ...
            dfs(v,u);
            isleaf=false;
            maxchild[u]=max(maxchild[u],maxchild[v]);
            if(maxdep[u][0].fs<=maxdep[v][0].fs+1)
            {
                maxdep[u][1]=maxdep[u][0];
                maxdep[u][0]=maxdep[v][0];
                maxdep[u][0].fs++;
            }
            else if(maxdep[u][1].fs<=maxdep[v][0].fs+1)
            {
                maxdep[u][1]=maxdep[v][0];
                maxdep[u][1].fs++;
            }
            totchild[u]+=totchild[v];
        }
    }
    if(isleaf) maxdep[u][0]=maxdep[u][1]=mp(0,u);
    int totdep[u]=maxdep[u][0].fs+maxdep[u][0].fs;
    return ;
}
